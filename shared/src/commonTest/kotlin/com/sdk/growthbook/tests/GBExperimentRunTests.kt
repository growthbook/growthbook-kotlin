    ...
    fun testExperiments(){
        ...
        for (item in evalConditions) {
            if (item is JsonArray) {
                ...
                val testContext = TestData.jsonParser.decodeFromJsonElement<GBContextTest>(item[1])
                val experiment = TestData.jsonParser.decodeFromJsonElement<GBExperiment>(item[2])
