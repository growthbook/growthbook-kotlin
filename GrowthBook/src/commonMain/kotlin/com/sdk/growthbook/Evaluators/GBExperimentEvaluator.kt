...
internal class GBExperimentEvaluator {
    ...
    fun evaluateExperiment(context: GBContext, experiment: GBExperiment) : GBExperimentResult{
        ...
        var bucketRange : List<GBBucketRange> = GBUtils.getBucketRanges(experiment.variations.size, experiment.coverage!!, experiment.weights!!)

        val hash = GBUtils.hash(attributeValue + experiment.key)
        var assigned = hash?.let { GBUtils.chooseVariation(it, bucketRange) } ?: -1
